...
import { X, Navigation, Fuel, AlertTriangle, CheckCircle2, MapPin, ArrowRight, Loader2 } from 'lucide-react';
...
import type { RouteWorker } from '../../workers/routeWorker';
...
const RouteOptimizationModal: React.FC<RouteOptimizationModalProps> = ({ isOpen, onClose, fields, forecast }) => {
    ...
    useEffect(() => {
        ...
        const computeRoute = async () => {
            ...
            try {
                ...
                const routeWorker = Comlink.wrap<RouteWorker>(worker);

                const result = await routeWorker.optimizeRoute(fields, forecast);

                if (active) {
                    setOptimization(result);
                }

                // Terminate worker when done to free memory, or keep it if frequent
                worker.terminate();
            } catch (error) {
                console.error("Failed to optimize route in worker:", error);
                // Fallback to synchronous if worker fails
                if (active) {
                    setOptimization(optimizeRoute(fields, forecast));
                }
            } finally {
    ...
    const routePositions = useMemo(() => optimization ? optimization.orderedFields.map(f => f.coordinates) : [], [optimization]);
                ...
                <div className="flex-1 flex flex-col md:grid md:grid-cols-[1fr_320px] overflow-hidden">
                    ...
                    {isOptimizing && (
                        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/80 dark:bg-neutral-900/80 backdrop-blur-sm">
                            <Loader2 className="animate-spin text-agro-green mb-4" size={48} />
                            <p className="font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest text-sm animate-pulse">
                                Inteligência Artificial em Curso...
                            </p>
                            <p className="text-[10px] text-gray-500 mt-2 uppercase">A processar limites de {fields.length} parcelas</p>
                        </div>
                    )}
                        ...
                        <div className="absolute bottom-4 left-4 z-20 flex flex-col gap-2">
                            <div className="bg-black/60 backdrop-blur-md px-3 py-2 rounded-xl text-white border border-white/20 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-black leading-none">{optimization?.totalFuelSaving || 0} L <span className="text-[10px] text-agro-green">Gasóleo</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar space-y-4">
                        ...
                        <div className="space-y-3 relative">
                            {optimization?.orderedFields.map((field, i) => (
                                <div key={field.id} className="relative">
                                    ...
                                    {optimization && i < optimization.orderedFields.length - 1 && (
                        ...
                        {optimization && optimization.weatherAlerts.length > 0 && (
