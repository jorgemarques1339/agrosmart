...
import {
  ...
  Package, Users, Clock, Coins, Camera, Image
} from 'lucide-react';
          ...
          {activeTab === 'entry' && (
            <div className="max-w-xl mx-auto h-full flex flex-col">
              ...
              <div className="flex items-center justify-between mb-8 px-4">
                {[1, 2, 3].map(s => (
                  <div key={s} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all ${step >= s ? 'bg-agro-green text-white' : 'bg-gray-200 text-gray-400'
                    }`}>
                    {s}
                  </div>
                ))}
              ...
              {step === 2 && (
                <div className="flex-1 space-y-5 animate-slide-up">
                  ...
                  {/* VISUAL EVIDENCE - Moved to Top */}
                  <div className="bg-white dark:bg-neutral-800 p-4 rounded-2xl border border-gray-100 dark:border-neutral-800 mb-4">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2">
                      <label className="w-16 h-16 rounded-xl border-2 border-dashed border-gray-300 dark:border-neutral-700 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-colors shrink-0">
                      ...
                      {entryData.attachments?.map((img, idx) => (
                        <div key={idx} className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 border border-gray-100 dark:border-neutral-700 group">
                          <img src={img} className="w-full h-full object-cover" alt="evidence" />
                          <button
                            onClick={() => setEntryData(prev => ({ ...prev, attachments: prev.attachments?.filter((_, i) => i !== idx) }))}
        ...
        {activeTab === 'records' && (
          <div className="p-6 bg-white dark:bg-neutral-900 border-t border-gray-100 dark:border-neutral-800">
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold text-white flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 ${filteredLogs.length === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 shadow-blue-600/30'
                }`}
            >
