...
import { 
  ...
  Trash2, AlertTriangle, Brain, Scale, Calendar // <--- Ícones adicionados
} from 'lucide-react';
...
export default function App() {
  ...
  const [confirmation, setConfirmation] = useState({
    ...
    message: '',
    onConfirm: null,
    isDanger: true // Se true, o botão é vermelho (para apagar)
  });

  // --- NOVO: Estados para Modal de Previsão IA ---
  ...
  // --- NOVA FUNÇÃO: CALCULAR PREVISÃO IA (SEM ALERT) ---
  const handlePredictYield = (field) => {
    if (!field.area || !field.cropCycle) {
      showNotification("Dados insuficientes. Adicione a área do campo.");
      return;
    }

    const yieldPerHa = field.cropCycle.yieldPerHa || 10;
    // Fatores de ajuste: Saúde da planta
    const healthFactor = field.health === 'Excelente' ? 1.2 : field.health === 'Bom' ? 1.0 : 0.7;
    
    const estimatedYield = (field.area * yieldPerHa * healthFactor).toFixed(1);
    
    setPredictionData({
      field: field.name,
      crop: field.cropCycle.label,
      img: field.img,
      area: field.area,
      health: field.health,
      harvestDate: field.cropCycle.harvest,
  ...
  const handleAddSale = () => {
    ...
    if (!isNaN(amount) && saleDesc.trim()) {
      ...
      setIsAddingSale(false); setSaleDesc(''); setSaleAmount(''); showNotification('Receita registada.');
    }
  };

  const handleAddStock = () => {
    if (!newStockData.name || !newStockData.quantity || !newStockData.price) { showNotification('Dados em falta.'); return; }
    const newStock = { id: `s${Date.now()}`, ...newStockData, quantity: parseFloat(newStockData.quantity), minLevel: parseFloat(newStockData.minLevel) || 0, price: parseFloat(newStockData.price) };
  ...
  const deleteField = (id) => {
    requestConfirmation(
      "Eliminar Campo",
      "Vai eliminar este campo e todo o seu histórico. Tem a certeza?",
      () => {
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onloadend = () => {
      ...
      setTimeout(() => {
        ...
        setAnalysisResult({ status: 'Atenção', disease: 'Míldio', treatment: 'Aplicar fungicida.', confidence: '94%' });
      }, 2500);
    };
  ...
  return (
    <div className="relative flex flex-col h-[100dvh] bg-[#FDFDF5] font-sans text-[#1A1C18] w-full max-w-md mx-auto shadow-2xl border-x border-gray-100 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-40 space-y-5 scroll-smooth w-full">
        ...
        {/* --- NOVO: MODAL DE PREVISÃO IA (DESIGN PREMIUM) --- */}
            ...
            <div className="bg-white rounded-t-[2.5rem] w-full max-w-md shadow-2xl animate-slide-up pb-10 border-t border-[#E0E4D6] overflow-hidden">
               <div className="bg-[#3E6837] p-8 text-white text-center relative overflow-hidden">
                 ...
                 <div className="relative z-10">
                   ...
                   <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest">Análise de Rendimento da Colheita</p>
                 </div>
               </div>
               ...
               <div className="p-6 space-y-6">
                 <div className="text-center">
                    ...
                    <div className="flex items-baseline justify-center gap-2 text-[#1A1C18]">
                       <span className="text-6xl font-black tracking-tighter">{predictionData.yield}</span>
                       <span className="text-sm font-black opacity-40 mb-2">TON</span>
                    </div>
                 </div>

                 <div className="grid grid-cols-2 gap-3">
                    <div className="bg-[#FDFDF5] p-4 rounded-[1.5rem] border border-[#EFF2E6] flex flex-col items-center">
                       <div className="flex items-center gap-2 mb-2 text-[#3E6837]"><Scale size={16} /><span className="text-[10px] font-black uppercase tracking-wide">Área Total</span></div>
                       <p className="text-xl font-bold text-[#1A1C18]">{predictionData.area} ha</p>
                    </div>
                    <div className="bg-[#FDFDF5] p-4 rounded-[1.5rem] border border-[#EFF2E6] flex flex-col items-center">
                       <div className="flex items-center gap-2 mb-2 text-[#3E6837]"><Activity size={16} /><span className="text-[10px] font-black uppercase tracking-wide">Saúde Atual</span></div>
                       <p className="text-xl font-bold text-[#1A1C18]">{predictionData.health}</p>
                    </div>
                    <div className="col-span-2 bg-[#FDFDF5] p-4 rounded-[1.5rem] border border-[#EFF2E6] flex flex-col items-center">
                       <div className="flex items-center gap-2 mb-2 text-[#3E6837]"><Calendar size={16} /><span className="text-[10px] font-black uppercase tracking-wide">Colheita Estimada</span></div>
                       <p className="text-xl font-bold text-[#1A1C18]">{predictionData.harvestDate}</p>
                    </div>
                 </div>

                 <button onClick={() => setIsPredictionOpen(false)} className="w-full py-5 bg-[#1A1C18] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all">
                    Fechar Relatório
                 </button>
               </div>
            </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-sm px-0 animate-fade-in">
            <div className="bg-white rounded-t-[2.5rem] p-6 w-full max-w-md shadow-2xl animate-slide-up pb-10 border-t border-[#E0E4D6]">
              ...
              <div className="text-center mb-6">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${confirmation.isDanger ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-500'}`}>
                  {confirmation.isDanger ? <Trash2 size={32} /> : <AlertTriangle size={32} />}
                </div>
                <h3 className="text-xl font-black text-[#1A1C18] mb-2 tracking-tight">{confirmation.title}</h3>
                <p className="text-sm text-[#43483E] px-4 leading-relaxed font-medium">{confirmation.message}</p>
              </div>
              <div className="flex gap-3">
                <button onClick={closeConfirmation} className="flex-1 py-4 border-2 border-[#E0E4D6] rounded-2xl font-black text-xs uppercase tracking-widest text-[#43483E] active:bg-gray-50 transition-colors">Cancelar</button>
                <button onClick={confirmation.onConfirm} className={`flex-1 py-4 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all ${confirmation.isDanger ? 'bg-red-600 shadow-red-200' : 'bg-blue-600 shadow-blue-200'}`}>{confirmation.isDanger ? 'Eliminar' : 'Confirmar'}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm px-0">
            <div className="bg-white rounded-t-[2.5rem] p-7 w-full max-w-md shadow-2xl animate-slide-up border-t border-[#E0E4D6] max-h-[85vh] overflow-y-auto">
              ...
              <div className="space-y-4 pb-10">
                ...
                <div className="flex gap-2">
                  <select className="flex-1 bg-[#FDFDF5] border-2 border-[#E0E4D6] rounded-2xl px-3 py-4 text-sm font-bold" value={newStockData.category} onChange={e => setNewStockData({...newStockData, category: e.target.value})}><option value="feed">Ração</option><option value="meds">Medicamento</option><option value="fertilizer">Adubo</option><option value="fuel">Combustível</option></select>
        ...
        {activeTab === 'animal' && (
          <div className="space-y-10 max-w-md mx-auto pb-4 pt-4">
            ...
            <div className="flex flex-col items-center justify-center gap-6 py-4">
              ...
              {!scannedAnimalId && (
                <button onClick={handleScanForAdd} className="flex items-center gap-2 bg-white px-6 py-3 rounded-2xl text-[#3E6837] font-bold shadow-sm border border-[#CBE6A2] hover:bg-[#E1E4D5] transition-all active:scale-95"><Plus size={18} /> Adicionar Tag Manual</button>
              )}
            </div>
            {scannedAnimal && <AnimalCard data={scannedAnimal} onAddProduction={addAnimalProduction} />}
            {scannedAnimalId && (
              <button onClick={() => setScannedAnimalId(null)} className="w-full py-5 text-[11px] font-black text-[#74796D] uppercase tracking-[0.3em] hover:text-[#3E6837] transition-colors active:bg-gray-100 rounded-2xl mt-2 mb-8">Nova Leitura / Limpar</button>
            )}
          </div>
        )}
          ...
          <div className="space-y-7 max-w-md mx-auto pb-4">
            ...
            <div className="flex items-center justify-between mt-6 px-1">
              ...
              <div className="flex gap-2.5">
                <label className="flex items-center gap-2.5 p-4 bg-white rounded-2xl border-2 border-[#E0E4D6] text-[#3E6837] cursor-pointer shadow-sm active:scale-90 active:bg-[#FDFDF5] transition-all px-5"><Camera size={22} /><span className="text-[10px] font-black uppercase tracking-widest">Usar IA</span><input type="file" className="hidden" accept="image/*" capture="environment" onChange={handleImageUpload} /></label>
                <button onClick={() => setIsAddingField(true)} className="bg-[#3E6837] text-white p-4 rounded-2xl active:scale-85 shadow-lg shadow-green-900/20 transition-all border-2 border-[#2D4F00]"><Plus size={26} /></button>
              </div>
            </div>
            <div className="grid gap-6">{cultivoView === 'list' ? fields.map(field => (<FieldCard key={field.id} field={field} isExpanded={expandedFieldId === field.id} onToggleHistory={() => setExpandedFieldId(expandedFieldId === field.id ? null : field.id)} logs={fieldLogs.filter(l => l.fieldId === field.id)} onAddLog={addFieldLog} onToggleIrrigation={toggleIrrigation} onPredictYield={handlePredictYield} onDelete={(id) => deleteField(id)} />)) : <FieldMap fields={fields} />}</div>
          </div>
        )}
        ...
        {activeTab === 'stocks' && (
          <div className="mt-4 space-y-4 max-w-md mx-auto">
            <div className="flex justify-between items-center px-2">
               ...
               <button onClick={() => setIsAddingStock(true)} className="bg-[#3E6837] text-white px-4 py-3 rounded-2xl text-xs font-bold flex items-center gap-2 shadow-lg active:scale-95 transition-all"><Plus size={18} /> Produto</button>
            </div>
        ...
        {activeTab === 'finance' && (
          <div className="mt-4 space-y-4 max-w-md mx-auto">
            <div className="flex items-center justify-between mt-6 px-1">
              ...
              <button onClick={() => setIsAddingSale(true)} className="bg-[#3E6837] text-white px-4 py-3 rounded-2xl text-xs font-bold flex items-center gap-2 shadow-lg active:scale-95 transition-all"><Plus size={18} /> Receita</button>
            </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-[#FDFDF5]/95 backdrop-blur-2xl h-20 pb-4 flex justify-around items-center z-40 border-t border-[#E0E4D6] px-2 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
        {[ {id: 'home', icon: Home, label: 'Início'}, {id: 'animal', icon: Scan, label: 'Animal'}, {id: 'cultivo', icon: Sprout, label: 'Cultivo'}, {id: 'stocks', icon: ClipboardList, label: 'Stocks'}, {id: 'finance', icon: Coins, label: 'Contas'} ].map(tab => (
          <button key={tab.id} onClick={() => { setActiveTab(tab.id); const container = document.querySelector('.overflow-y-auto'); if (container) container.scrollTo({ top: 0, behavior: 'smooth' }); }} className="flex flex-col items-center gap-1 flex-1 py-1 relative active:scale-90 transition-transform">
            <div className={`h-9 w-14 rounded-xl flex items-center justify-center transition-all duration-300 ${activeTab === tab.id ? 'bg-[#3E6837] text-white shadow-md' : 'bg-transparent text-[#74796D]'}`}><tab.icon size={26} strokeWidth={activeTab === tab.id ? 2.5 : 2} /></div>
            ...
            {activeTab === tab.id && <div className="absolute -bottom-1 w-2 h-2 bg-[#3E6837] rounded-full animate-bounce shadow-md"></div>}
          </button>
        ))}
      </div>
    </div>
